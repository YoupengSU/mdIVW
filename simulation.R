...
# Section 5.2 A full comparison among the dIVW, pIVW, and mdIVW estimators----------------
...
#** Figure 2b (Empirical standatd error)--------------------------------------------------
data_ana <-  data1[data1$trim=="t99",]
...
#** Figure 2d (Coverage probability)------------------------------------------------------
data_ana <- data1[data1$trim=="t99",]
...
#** Figure 3b (Empirical standard error)--------------------------------------------------
data_SD <- data_ana[c("SD_d","SD_m","SD_p","SE_d","SE_m","SE_p","eta","kappa","eta_true","kappa_true")]
...
#** Figure 3d (Coverage probability)------------------------------------------------------
...
# Section 5.3 Compared with other commenly used methods-----------------------------------
# tau=0.01
# delta = 3.7
...
#* Conduct simulation---------------------------------------------------------------------
#* Table 1 in the main text (no pleiotropy and no IV selection)---------------------------
...
cl <- makeCluster(8)
registerDoParallel(cl)
out <- foreach(x=1:20000,.combine='rbind') %dopar% 
  sim_parallel_table(tau=0,delta=0,data_true = data_true) # tau = 0.01 for balanced horizontal pleiotropy
stopCluster(cl)
time_end <- Sys.time()
time_end - time_start
...
#* Table 2 in the mian text (no pleiotropy but with IV selection--------------------------
...
out <- foreach(x=1:2000,.combine='rbind') %dopar% 
  sim_parallel_table(tau=0,delta=3.7,data_true = data_true) # tau = 0.01 for balanced horizontal pleiotropy
stopCluster(cl)
time_end <- Sys.time()
time_end - time_start
